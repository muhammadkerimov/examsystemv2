...
def exams(request):
   ...
   if (len(Exam_Dat.objects.all().filter(exam_grade=request.session.get('sinif')).values())>0):
      datas =  Exam_Dat.objects.all().filter(exam_grade=int(request.session.get('sinif'))).values()
      for data in datas:
       regdat = data['exam_time']
       data['exam_time'] = data['exam_time'].strftime('%Y-%m-%d %H:%M:%S')
       data['exam_time'] = datetime.strptime(data['exam_time'], '%Y-%m-%d %H:%M:%S')
       a = datas.filter(exam_time=regdat)
       a.update(exam_time=data['exam_time'])
      context  = {
         'now':datetime.now(),
         'exams':datas
      }
...
def examstart(request):
   if request.method == "POST" and request.POST.get('end'):
      questions = Exam_Dat.objects.all().filter(exam_id=request.POST.get('examid')).values()[0]['questions']
      ...
      exam = Exam_Dat.objects.all().filter(exam_name=request.POST['start']).values_list('participated_childrens','exam_id')
      exam = exam[0][0]
      exam = exam.split(';')
      ...
      return render(request,"examnotaviable.html")
...
def result(request):
   if request.method == "POST":
    ...
    dataresults = Children_Results.objects.all().filter(child_email=request.session.get('mail'),exam_id=request.POST.get('end')).values()
